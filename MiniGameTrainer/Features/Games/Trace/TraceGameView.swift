...
struct TraceGameView: View {
    ...
    var body: some View {
        TraceGameContentView(
            viewModel: TraceGameViewModel(
...
private struct TraceGameContentView: View {
    ...
    init(viewModel: @autoclosure @escaping () -> TraceGameViewModel) {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack {
                if let scene = viewModel.scene(for: fullSize) {
                    ...
                    .ignoresSafeArea()
                    .accessibilityLabel("Trace game area. Memorize the yellow path, then drag through the dots.")
                } else {
        ...
        .background(Color(TraceGameConfig.reference.backgroundColor).ignoresSafeArea())
        .onAppear {
...
private struct TracePauseOverlay: View {
    let onResume: () -> Void
    let onRestart: () -> Void
    let onQuit: () -> Void
