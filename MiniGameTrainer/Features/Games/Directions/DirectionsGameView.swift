...
struct DirectionsGameView: View {
    ...
    var body: some View {
        DirectionsGameContentView(
            viewModel: DirectionsGameViewModel(
                config: tuning.resolvedConfig(),
...
private struct DirectionsGameContentView: View {
    @StateObject private var viewModel: DirectionsGameViewModel
    @EnvironmentObject private var router: AppRouter
    @EnvironmentObject private var statistics: StatisticsStore
    @Environment(\.scenePhase) private var scenePhase

    init(viewModel: @autoclosure @escaping () -> DirectionsGameViewModel) {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack {
                if let scene = viewModel.scene(for: fullSize) {
                    ...
                    .ignoresSafeArea()
                    .accessibilityLabel("Directions game area. Watch the arrows, then reproduce the sequence.")
                } else {
        ...
        .background(Color(UIColor(red: 46 / 255, green: 138 / 255, blue: 230 / 255, alpha: 1)).ignoresSafeArea())
        .onAppear {
...
private struct DirectionsPauseOverlay: View {
    let onResume: () -> Void
    let onRestart: () -> Void
    let onQuit: () -> Void
        ...
        ZStack {
            ...
            VStack(spacing: 16) {
                Text("Paused")
                    ...
                    .padding(.bottom, 8)
                Text("The current sequence will restart. A partial memory trial is not scored.")
