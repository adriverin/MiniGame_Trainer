...
struct TrampboxGameView: View {
    @EnvironmentObject private var environment: AppEnvironment
    @ObservedObject private var tuning = TrampboxTuningStore.shared

    var body: some View {
        TrampboxGameContentView(
            viewModel: TrampboxGameViewModel(
...
private struct TrampboxGameContentView: View {
    @StateObject private var viewModel: TrampboxGameViewModel
    @EnvironmentObject private var router: AppRouter
    @EnvironmentObject private var statistics: StatisticsStore
    @Environment(\.scenePhase) private var scenePhase

    init(viewModel: @autoclosure @escaping () -> TrampboxGameViewModel) {
        ...
        GeometryReader { proxy in
            ...
            ZStack {
                if let scene = viewModel.scene(for: fullSize) {
                    ...
                    .ignoresSafeArea()
                    .accessibilityLabel("Trampbox game area. Drag left and right to steer.")
                } else {
...
private struct TrampboxPauseOverlay: View {
    let onResume: () -> Void
    let onRestart: () -> Void
    let onQuit: () -> Void
