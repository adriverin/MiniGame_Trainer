...
struct SwipeFastGameView: View {
    @EnvironmentObject private var environment: AppEnvironment
    @ObservedObject private var tuning = SwipeFastTuningStore.shared

    var body: some View {
        SwipeFastGameContentView(
            viewModel: SwipeFastGameViewModel(
...
private struct SwipeFastGameContentView: View {
    @StateObject private var viewModel: SwipeFastGameViewModel
    @EnvironmentObject private var router: AppRouter
    @EnvironmentObject private var statistics: StatisticsStore
    @Environment(\.scenePhase) private var scenePhase

    init(viewModel: @autoclosure @escaping () -> SwipeFastGameViewModel) {
        ...
        GeometryReader { proxy in
            ...
            ZStack {
                if let scene = viewModel.scene(for: fullSize) {
                    ...
                    .ignoresSafeArea()
                    .accessibilityLabel("Swipe Fast game area. Swipe inside each box in the direction of its arrow.")
                } else {
...
private struct SwipeFastPauseOverlay: View {
    let onResume: () -> Void
    let onRestart: () -> Void
    let onQuit: () -> Void
        ...
        ZStack {
            ...
            VStack(spacing: 16) {
                Text("Paused")
                    ...
                    .padding(.bottom, 8)
                Text("Box timers restart when you come back. A backgrounded run is not scored.")
