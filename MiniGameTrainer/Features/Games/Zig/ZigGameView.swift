...
struct ZigGameView: View {
    @EnvironmentObject private var environment: AppEnvironment
    @ObservedObject private var tuning = ZigTuningStore.shared

    var body: some View {
        ZigGameContentView(viewModel: ZigGameViewModel(
...
private struct ZigGameContentView: View {
    @StateObject private var viewModel: ZigGameViewModel
    @EnvironmentObject private var router: AppRouter
    @EnvironmentObject private var statistics: StatisticsStore
    @Environment(\.scenePhase) private var scenePhase

    init(viewModel: @autoclosure @escaping () -> ZigGameViewModel) {
    ...
    var body: some View {
            ...
            ZStack {
                if let scene = viewModel.scene(for: fullSize) {
                    SpriteView(scene: scene, preferredFramesPerSecond: UIScreen.main.maximumFramesPerSecond,
                        ...
                        .ignoresSafeArea()
                        .accessibilityLabel("ZIG game area. Tap anywhere to change the ball's direction.")
                } else {
                    Color(ZigGameConfig.reference.backgroundBottomColor).ignoresSafeArea()
                }
                VStack {
                    HStack {
                        Spacer()
        ...
        .background(Color(ZigGameConfig.reference.backgroundBottomColor).ignoresSafeArea())
        .onAppear {
            viewModel.onFinish = { result in GameSessionHost(router: router, statistics: statistics).finish(result) }
        }
...
private struct ZigPauseOverlay: View {
    let onResume: () -> Void
    let onRestart: () -> Void
    let onQuit: () -> Void
