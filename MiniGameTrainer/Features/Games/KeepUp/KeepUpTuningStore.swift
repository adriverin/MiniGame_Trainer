...
final class KeepUpTuningStore: ObservableObject {
    ...
    private init() {
        ...
        if let index = arguments.firstIndex(of: "-keepUpPhysicsScore"), arguments.indices.contains(index + 1),
           let value = Int(arguments[index + 1]) {
            debugOptions.physicsScoreOverride = max(0, value)
            debugOptions.autoCatchPlatformYRatio = config.startingPlatformYRatio
        }
