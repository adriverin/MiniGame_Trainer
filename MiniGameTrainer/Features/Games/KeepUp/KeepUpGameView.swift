...
struct KeepUpGameView: View {
    @EnvironmentObject private var environment: AppEnvironment
    @ObservedObject private var tuning = KeepUpTuningStore.shared

    var body: some View {
        KeepUpGameContentView(
            viewModel: KeepUpGameViewModel(
...
private struct KeepUpGameContentView: View {
    @StateObject private var viewModel: KeepUpGameViewModel
    @EnvironmentObject private var router: AppRouter
    @EnvironmentObject private var statistics: StatisticsStore
    @Environment(\.scenePhase) private var scenePhase

    init(viewModel: @autoclosure @escaping () -> KeepUpGameViewModel) {
        ...
        GeometryReader { proxy in
            ...
            ZStack {
                if let scene = viewModel.scene(for: fullSize) {
                    ...
                    .ignoresSafeArea()
                    .accessibilityLabel("Keep Up game area. Drag to move the platform under the ball.")
                } else {
...
private struct KeepUpPauseOverlay: View {
    let onResume: () -> Void
    let onRestart: () -> Void
    let onQuit: () -> Void
